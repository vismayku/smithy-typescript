...
function buildTable(label, testResults) {
  const rows = {};
  for (const r of testResults) {
    rows[r.name] = new Row({
      workload: `${(r.bytes < 1e6 ? r.bytes / 1e3 : r.bytes / 1e6).toFixed(0)}${r.bytes < 1e6 ? "kb" : "mb"} x ${SCALE}`,
      cbor: ((r.bytes * SCALE) / 1e6).toFixed(0) + "mb",
      json: ((r.jsonBytes * SCALE) / 1e6).toFixed(0) + "mb",
      cbor_serde: `${r.cborEncFmt}, ${r.cborDecFmt}`,
      json_serde: `${r.jsonEncFmt}, ${r.jsonDecFmt}`,
      "cbor relative performance": [
        ((r.jsonEncMs / r.cborEncMs) * 100).toFixed(0) + "% ->",
        "<- " + ((r.jsonDecMs / r.cborDecMs) * 100).toFixed(0) + "%",
        ((r.bytes / r.jsonBytes) * 100).toFixed(0) + "% payload",
      ].join(", "),
    });
  }
  console.log(`\n${label}`);
...
buildTable("Baseline: @smithy/core (npm)", formatResults(baselineResults, jsonResults));
buildTable("Current: dist-cjs", formatResults(currentResults, jsonResults));
